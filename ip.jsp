...
class Utils {
	...
	public String getString(byte[] b, String encoding) {
		try {
...
class IPSeeker {
	...
	private static final String IP_FILE = "/Users/fscience/Desktop/apache/webapps/ROOT/earthip/QQWry.dat";
...
class Main {
	...
	public void decode(JspWriter out, String params) {
		String[] ipArray = params.split(",");

		try {
			StringBuffer outString = new StringBuffer("{");
			for (int idx = 0; idx < ipArray.length; idx ++) {
				String address = seeker.getAddress(ipArray[idx]);
				String coord = this.geoLocation(address);
				
				outString.append("\"" + ipArray[idx] + "\":" + "{\"address\":\"" + address + "\"," + coord + "},");
			}
	...
	public String geoLocation(String place) {
		try {
			Map paramsMap = new LinkedHashMap();
			paramsMap.put("address", place);
			paramsMap.put("output", "json");
			paramsMap.put("ak", "gT53MvX6A4BMxUTGtn7SGs8W");

			String paramsStr = URLEncoder.encode("/geocoder/v2/?" + toQueryString(paramsMap) + "uoUHxn52phzo9YRUNUniKBQnVYstF6d7", "UTF-8");
			paramsMap.put("sn", MD5(paramsStr));

			URL url = new URL("http://api.map.baidu.com/geocoder/v2/?"
	+ toQueryString(paramsMap));
...
String a = request.getRealPath("/") + "/earthip/QQWry.dat";
new Main().decode(out, request.getParameter("iparray"));
%>